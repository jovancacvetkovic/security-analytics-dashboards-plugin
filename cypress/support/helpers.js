...
Cypress.Commands.add('getFieldByLabel', (label, type = 'input') => {
  ...
  return cy.getElementByText('.euiFormRow__labelWrapper', label).siblings().find(type);
});
...
Cypress.Commands.add(
  ...
  (subject, items) => {
    ...
    items.map((item) => cy.wrap(subject).type(`${item}{enter}`));
  }
);
  ...
  (subject) => {
    ...
    return cy
      ...
      .then(($badge) => {
        ...
        _.times(
          cy.wrap(subject).parents('.euiComboBox__inputWrap').find('input').type('{backspace}'),
          numberOfBadges
        );
      });
  }
